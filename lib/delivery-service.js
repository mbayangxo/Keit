...
export function deliveryListShape(task, { riderLat, riderLng, viewerId, accepted } = {}) {
  const distance =
...
export async function listNearbyDeliveries(db, { riderLat, riderLng, viewerId }) {
  ...
  const shaped = tasks.map((task) =>
    deliveryListShape(task, { riderLat, riderLng, viewerId, accepted: false }),
  );
...
export async function getDeliveryDetail(db, taskId, viewerId) {
  const task = await db.deliveryTask.findUnique({
    where: { id: taskId },
    include: { escrow: true, dispute: { include: { evidence: true } } },
  });
  ...
  return {
    ...deliveryListShape(task, { viewerId, accepted }),

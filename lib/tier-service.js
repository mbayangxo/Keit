...
export async function assertCanCreateBusiness(user) {
  ...
  if (!limits.canCreateBusiness) {
    throw new TierLimitError(
      ...
      'Comptes professionnels disponibles au Tier 3 (adresse vérifiée)',
    );
  }
}
...
export async function assertWalletWithinCaps(user, wallet, { incomingNational = 0, incomingKori = 0 } = {}) {
  const limits = limitsForTier(effectiveTier(user));
  const nextNational = wallet.balance + incomingNational;
  const nextKori = wallet.koriBalance + incomingKori;

  if (nextNational > limits.maxNationalBalance) {
    throw new TierLimitError(
      'tier_balance_cap',
      `Plafond Tier ${effectiveTier(user)} : ${limits.maxNationalBalance.toLocaleString('fr-FR')} XOF maximum`,
    );
  }
  if (nextKori > limits.maxKoriBalance) {
    throw new TierLimitError(
      'tier_balance_cap',
      `Plafond Tier ${effectiveTier(user)} : ₭${limits.maxKoriBalance.toLocaleString('fr-FR')} maximum`,
    );
  }
}

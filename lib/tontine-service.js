...
export async function processTontineGroup(groupId, db = prisma, now = new Date()) {
  ...
  const expectedPotKori = legacyNationalToKori(expectedPot, 'SN');
  ...
  await runMoneyTransaction(db, async (tx) => {
    ...
    for (const membership of group.memberships) {
      ...
      const amountKori = legacyNationalToKori(group.amountPerMember, membership.user.country ?? 'SN');
      ...
      try {
        ...
        collections.push({ userId: membership.userId, amount: amountKori, ok: true });
      } catch (error) {
        collectionFailed = true;
        collections.push({

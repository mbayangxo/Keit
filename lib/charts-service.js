...
/**
 * K21 Charts — the community favorite-song chart ("Wey yu 221 bëgg").
...
function validateSongInput(title, artist, videoId) {
  ...
  // Songs are picked from YouTube search — never free-typed — so a real
  // video id is mandatory.
  if (!/^[A-Za-z0-9_-]{5,20}$/.test(v)) throw new ChartsError('invalid_video', 'Choisis une chanson depuis la recherche YouTube.');
  return { title: t, artist: a, videoId: v };
}
...
export async function submitAndVote(userId, { title, artist, videoId } = {}) {
  const clean = validateSongInput(title, artist, videoId);

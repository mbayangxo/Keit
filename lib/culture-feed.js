...
export async function getCultureFeed({ country = 'SN', query = '' } = {}) {
  const eventbrite = await getEventbriteEvents();
  const items = [...eventbrite, ...(BASE_BY_COUNTRY[country] ?? BASE_BY_COUNTRY.SN)];
  ...
  return {
    ...
    items: filtered,
    preview: true,
    source: eventbrite.length > 0 ? 'eventbrite+curated' : 'curated',

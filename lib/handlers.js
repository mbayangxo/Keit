...
import { profileShape, publicProfileShape, recipientLookupShape, txShape } from './shapes.js';
import {
  hashToken,
  otp,
  reference,
...
export async function getMe(req, res) {
  ...
  const worker = await getWorkerProfile(req.userId);
  res.json(
    profileShape(user, {
...
export async function transfersSend(req, res) {
  ...
  const { recipientHandle, note } = parsed.data;
...
export async function transfersRequest(req, res) {
  ...
  const { recipientHandle, note, voiceNoteUrl } = parsed.data;
      ...
      await Promise.all(
        others.map((m) =>
          createInAppNotification(
            ...
            `${callerName} t’appelle sur Mboolo — ouvre la conversation pour répondre.`,
          ),
        ),
      );
    }
...
export async function publicProfileGet(req, res) {
  ...
  const [ngor, poll, communityConfirmed] = await Promise.all([
    computeNgorScore(user.id).catch(() => 0),
    activePollShape(user.id, req.userId).catch(() => null),
    isConfirmed(user.id).catch(() => false),
  ]);
  res.json(publicProfileShape(user, { ngor, poll, communityConfirmed }));
}

...
import { JULAYA_OPERATORS } from './julaya.js';
...
const emailAuthBody = z.object({
  ...
  intent: z.enum(['login']).optional().default('login'),
});
...
async function findValidOtpForKeys(keys, code) {
  ...
  if (latest && latest.code === trimmedCode) return latest;
  return null;
}
...
/** Sign in with email — OTP sent to address on file (for users who changed phone numbers). */
export async function authEmail(req, res) {
  ...
  if (!user) {
    res.status(404).json({
      error: 'Aucun compte K21 pour cet email — crée un compte d\'abord ou utilise ton numéro.',
    });
    return;
  }
  ...
  const allowBetaOtp = process.env.ALLOW_BETA_OTP === 'true';
  const hideOtp = process.env.NODE_ENV === 'production' && !allowBetaOtp && emailConfigured();

  if (hideOtp) {
    res.json({ sent: true, message: 'Code envoyé par email', emailSent: true });
  } else {
    res.json({
      otp: code,
...
export async function authVerify(req, res) {
  ...
  if (!stored) {
    res.status(401).json({
      ...
      ...(process.env.ALLOW_BETA_OTP === 'true'
        ? { hint: 'Request a new code. Use the latest orange code.', ...hintMeta }
        : {}),
    });
    return;
  }
  ...
  if (!user) {
    if (intent === 'login' || intent === 'recover' || isEmailLogin) {
      res.status(404).json({
        error: isEmailLogin
...
export async function cultureFeed(req, res) {
  ...
  res.json(getCultureFeed({ country, query: q }));
}
...
export async function mboloThreadMessages(req, res) {
  ...
  if ((kind === 'voice' || kind === 'image') && !mediaUrl) {
    res.status(400).json({ error: 'mediaUrl required for voice/image messages' });
    return;
  }
  ...
  const message = await prisma.mboloMessage.create({
    data: {
      threadId,
      senderId: req.userId,
      body: body?.trim() ?? (kind === 'image' ? '📷 Photo' : kind === 'voice' ? '🎤 Message vocal' : ''),
      kind,

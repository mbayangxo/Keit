...
export async function transfersSend(req, res) {
  ...
  const { recipientHandle, note, voiceNoteUrl, photoUrl, videoUrl, giftCardTheme } = parsed.data;
  const attachmentUrl = voiceNoteUrl ?? photoUrl ?? videoUrl ?? null;
  const attachmentType = voiceNoteUrl ? 'voice' : photoUrl ? 'photo' : videoUrl ? 'video' : null;

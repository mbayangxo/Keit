...
export function agentDiscoveryShape(agent, { lat, lng } = {}) {
  ...
  return {
    ...
    canServe: agent.status === 'active' && agent.floatBalance >= 500,
    openNow: agent.status === 'active' && agent.floatBalance >= 500,
  };
}

export async function listNearbyAgents(db = prisma, { lat, lng, limit = 30 } = {}) {
  ...
  const shaped = agents.map((a) => agentDiscoveryShape(a, { lat, lng }));
  ...
  return shaped.filter((a) => a.canServe).slice(0, limit);
}

  ...
  for (const group of due) {
    ...
    try {
      await runMoneyTransaction(db, async (tx) => {
        ...
        // Members' dues land in the creator's own wallet — it doubles as the
        // pot, and may also hold the creator's unrelated personal balance.
        // The payout must use the known dues total (amountPerMember ×
        // memberCount), never the pot wallet's live balance, which would
        // leak the creator's own money into another member's payout.
        const collectedThisRound = collectionFailed
          ? collections
              .filter((c) => c.ok && c.userId !== group.createdBy)

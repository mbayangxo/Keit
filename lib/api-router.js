  ...
  try {
    if (matched.route.auth) {
      ...
      if (!userId) {
        ...
        // Beta: echo the precise internal reason. A non-technical founder
        // can only ever hand us a screenshot — this makes that screenshot
        // conclusive instead of another guessing round.
        const debug = process.env.ALLOW_BETA_OTP === 'true' ? authErr?.debug : undefined;

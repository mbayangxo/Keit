...
export async function settleRailFromWebhook(db, params) {
  ...
  if (failed) {
    return runMoneyTransaction(db, async (tx) => {
      await tx.$executeRaw`SELECT id FROM "RailTransaction" WHERE id = ${rail.id} FOR UPDATE`;
      const locked = await tx.railTransaction.findUniqueOrThrow({ where: { id: rail.id } });
      if (locked.status === 'failed' || locked.status === 'completed') return locked;

      if (locked.direction === 'out' && locked.walletDebited) {
        const koriRefund = nationalToKori(locked.amount, user.country ?? 'SN');
        await creditNational(tx, {
          walletId: user.wallet.id,
          userId: user.id,
          amount: koriRefund,
          reference: `${locked.reference}-REFUND`,
          ledger: {

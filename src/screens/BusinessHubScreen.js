            ...
            <View style={styles.panel}>
              ...
              {transactions.slice(0, 5).map((tx) => (
                <View key={tx.key} style={styles.txRow}>
                  ...
                  <Text style={[styles.txAmt, { color: tx.amount > 0 ? colors.flagGold : colors.flagRed }]}>
          ...
          {tab === 'payroll' && (
            <View style={styles.panel}>
              <SectionCard title="Ajouter un employé">
                <TextInput style={styles.input} placeholder="@handle K21" placeholderTextColor={colors.whiteA30} value={newHandle} onChangeText={setNewHandle} autoCapitalize="none" />
                <TextInput style={styles.input} placeholder="Rôle (staff, CEO…)" placeholderTextColor={colors.whiteA30} value={newRole} onChangeText={setNewRole} />
                <TextInput style={styles.input} placeholder="Salaire XOF" placeholderTextColor={colors.whiteA30} keyboardType="number-pad" value={newPay} onChangeText={setNewPay} />
          ...
          {tab === 'school' && (
            <View style={styles.panel}>
              <SectionCard title="Inscrire un élève">
                <TextInput style={styles.input} placeholder="Nom de l'enfant" placeholderTextColor={colors.whiteA30} value={newStudentName} onChangeText={setNewStudentName} />
                <TextInput style={styles.input} placeholder="@handle du parent" placeholderTextColor={colors.whiteA30} value={newParentHandle} onChangeText={setNewParentHandle} autoCapitalize="none" />
                <GlowButton label="Inscrire" onPress={enrollStudent} disabled={loading} />
              </SectionCard>
              <SectionCard title="Frais mensuels">
                <TextInput style={styles.input} placeholder="Ex: Mars 2026" placeholderTextColor={colors.whiteA30} value={periodLabel} onChangeText={setPeriodLabel} />
                <TextInput style={styles.input} placeholder="Montant XOF" placeholderTextColor={colors.whiteA30} keyboardType="number-pad" value={periodAmount} onChangeText={setPeriodAmount} />
          ...
          {tab === 'coop' && (
            <View style={styles.panel}>
              <SectionCard title="Log livraison (offline OK)">
                <TextInput style={styles.input} placeholder="ID ou @handle paysan" placeholderTextColor={colors.whiteA30} value={farmerHandle} onChangeText={setFarmerHandle} />
                <TextInput style={styles.input} placeholder="Tonnes" placeholderTextColor={colors.whiteA30} keyboardType="decimal-pad" value={deliveryTons} onChangeText={setDeliveryTons} />
...
const styles = StyleSheet.create({
  ...
  emptyTitle: { fontFamily: fontFamily.displayBold, fontSize: 18, color: colors.white },
  emptySub: { fontSize: 12, color: colors.whiteA40, marginTop: spacing.sm, textAlign: 'center' },
  ...
  bizName: { fontFamily: fontFamily.displayBold, fontSize: 16, color: colors.white },
  bizMeta: { fontSize: 11, color: colors.whiteA40, marginTop: 2 },
  notifBtn: { width: 36, height: 36, borderRadius: radius.lg, backgroundColor: colors.whiteA08, alignItems: 'center', justifyContent: 'center' },
  bizChip: { paddingHorizontal: spacing.lg, paddingVertical: spacing.sm, borderRadius: radius.round, backgroundColor: colors.whiteA06, marginRight: spacing.sm },
  bizChipOn: { backgroundColor: colors.goldA20 },
  bizChipText: { fontSize: 11, color: colors.whiteA50 },
  bizChipTextOn: { color: colors.flagGold, fontWeight: '700' },
  balance: { fontFamily: fontFamily.displayBlack, fontSize: 36, color: colors.flagGold, letterSpacing: -1 },
  balanceLabel: { fontSize: 11, color: colors.whiteA35 },
  tabRow: { paddingHorizontal: spacing.lg, paddingVertical: spacing.lg, gap: spacing.sm },
  tab: { paddingHorizontal: spacing.lg, paddingVertical: spacing.sm, borderRadius: radius.round, backgroundColor: colors.whiteA06, marginRight: spacing.sm },
  tabOn: { backgroundColor: colors.goldA20 },
  tabText: { fontSize: 11, fontWeight: '700', color: colors.whiteA45 },
  tabTextOn: { color: colors.flagGold },
  ...
  actionLabel: { fontSize: 9, color: colors.whiteA50, fontWeight: '700' },
  sectionEyebrow: { fontSize: 9, fontWeight: '700', letterSpacing: 1.2, color: colors.whiteA30, textTransform: 'uppercase', marginBottom: spacing.sm },
  txRow: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: spacing.sm, borderBottomWidth: 1, borderBottomColor: colors.whiteA06 },
  txTitle: { fontSize: 12, color: colors.white },
  txAmt: { fontFamily: fontFamily.bodyBold, fontSize: 12 },
  card: { backgroundColor: colors.whiteA04, borderRadius: radius.xl, borderWidth: 1, borderColor: colors.whiteA08, padding: spacing.lg, gap: spacing.sm },
  cardTitle: { fontFamily: fontFamily.bodyBold, fontSize: 12, color: colors.flagGold, marginBottom: spacing.xs },
  input: { height: 44, borderRadius: radius.lg, backgroundColor: colors.whiteA06, borderWidth: 1, borderColor: colors.whiteA10, paddingHorizontal: spacing.lg, color: colors.white, fontSize: 13 },
  listRow: { flexDirection: 'row', alignItems: 'center', gap: spacing.md, paddingVertical: spacing.sm, borderBottomWidth: 1, borderBottomColor: colors.whiteA06 },
  rowTitle: { fontFamily: fontFamily.bodyBold, fontSize: 12, color: colors.white },
  rowSub: { fontSize: 10, color: colors.whiteA40, marginTop: 2 },
  payChip: { backgroundColor: colors.greenA12, paddingHorizontal: spacing.md, paddingVertical: spacing.sm, borderRadius: radius.lg },
  payChipText: { fontSize: 10, fontWeight: '800', color: colors.green },
});
